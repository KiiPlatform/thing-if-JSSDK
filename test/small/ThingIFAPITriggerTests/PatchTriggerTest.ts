    ...
    describe("handle http response", function() {
        ...
        describe("hanle success response", function(){
            ...
            it("test callback", function (done) {
                thingIFAPI.patchCommandTrigger(triggerID,request,(err, trigger)=>{
                    try{
                        expect(err).to.null;
                        expect(trigger).to.be.deep.equal(expectedTrigger);
                        done();
        ...
        describe("handle err reponse", function() {
            ...
            it("test callback", function (done) {
                thingIFAPI.patchCommandTrigger(triggerID,request,(err, cmd)=>{
                    try{
                        expect(err).to.be.deep.equal(expectedError);
                        expect(cmd).to.null;
                        done();
...
describe("Small Test ThingIFAPI#patchServerCodeTrigger", function() {
    ...
    describe("handle IllegalStateError", function() {
        ...
        it("when targe is null, IllegalStateError should be returned(promise)",
            function (done) {
            thingIFAPI.patchServerCodeTrigger(triggerID,request)
            .then((trigger: Trigger)=>{
                done("should fail");
            }).catch((err)=>{
                expect(err.name).to.equal(Errors.IlllegalStateError);
    ...
    describe("handle http response", function() {
        ...
        describe("hanle success response", function(){
            ...
            it("test callback", function (done) {
                thingIFAPI.patchServerCodeTrigger(triggerID,request,(err, trigger)=>{
                    try{
                        expect(err).to.null;
                        expect(trigger).to.be.deep.equal(expectedTrigger);
                        done();
        ...
        describe("handle err reponse", function() {
            ...
            it("test callback", function (done) {
                thingIFAPI.patchServerCodeTrigger(triggerID,request,(err, cmd)=>{
                    try{
                        expect(err).to.be.deep.equal(expectedError);
                        expect(cmd).to.null;
                        done();
